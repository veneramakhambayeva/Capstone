#!/bin/bash
DIR=path/to/the/docker-sec/files
...
apply_cap_train(){
	...
	do
	        ...
	        if [[ -z $(grep ${cap} ${prof_path}) ]]; then
	        	info_out "Capstone: adding ${cap} capability to ${profile} profile"
...
apply_add(){
	if [ -z $1 ]; then
                echo "usage: ${bold_text}apply_add${normal_text} CONTAINER_RUNTIME_PROFILE" >&2
                return 1
        fi
	...
	grep "apparmor=\"DENIED\" operation=\"capable\" profile=\"${profile}\"" /var/log/audit/audit.log |
	while read line
	do
	        cap=$(echo ${line} | sed -r 's/.*capname="(.*)"/\1/')
	        if [[ -z $(grep ${cap} ${prof_path}) ]]; then
...
apply_net_cap_train(){
	...
	do
	        if [[ -z $(grep "${net_rule}" ${prof_path}) ]]; then
	        	info_out "Capstone: adding the network rule: \"${net_rule}\" to ${profile} profile"
...
apply_file_cap_train(){
	if [ -z $1 ]; then
                echo "usage: ${bold_text}apply_file_cap_train${normal_text} CONTAINER_RUNTIME_PROFILE" >&2
                return 1
        fi
	...
	if [ -e $DIR/vmm ]; then
		prompt_yes_no "Are you sure u want to remove files [Y/N]: " &&
		cp $DIR/vmm_bak $DIR/vmm
	fi
	...
	grep "operation=\"open\"" |
	grep "/dev/tty" |
	sed -r 's/.*comm="(.*)" requested_mask.*/\1/'|
	sort -u |
	while read line
	do
        cap=$(echo ${line}) 
        #echo $cap
        if [ -n "grep -w $cap $DIR/shells" ] ; then
			grep -w $cap $DIR/shells |
				while read l
				do
					t=$(echo ${l})
					sed -i "s!^${t}!!" $DIR/shells
					sed -i '/^$/d' $DIR/shells
				done
		else
			echo no $cap
	...
	cat $DIR/shells |
        ...
        do
            sh=$(echo ${line})
            sed -i "1 a\ deny ${sh} mrwklx," $DIR/profile
        done

	cat $DIR/profile |
	while read shell_rule
	do
	        if [[ -z $(grep "${shell_rule}" ${prof_path}) ]]; then
	        	info_out "Capstone: adding the shell rule: \"${shell_rule}\" to ${profile} profile"
	                sed -i "/shell_placeholder,/ a\  ${shell_rule}" ${prof_path}
	        fi
	done
	...
	grep -vw '/dev/tty'  |
	grep -vw 'aufs' |
	grep requested_mask=\"w |
	sed -r 's/.*name="(.*)" pid=.*/ \1 w,/' |
	sort -u |
	while read file_rule
	do
	        if [[ -z $(grep "${file_rule}" ${prof_path}) ]]; then
	        	info_out "Capstone: adding the file rule: \"${file_rule}\" to ${profile} profile"
	                sed -i "/file_placeholder,/ a\  ${file_rule}" ${prof_path}
					sed -i "/file_placeholder,/ a\  ${file_rule}" $DIR/vmm
	        fi
	done

	if [ -n $(cat /var/log/audit/audit.log | grep -vw aufs | grep -vw dev| grep requested_mask=\"wrd|  sed -r 's/.*name="(.*)" pid=.*/\1/' | sort -u) ]; then
        cat /var/log/audit/audit.log | grep -vw aufs | grep -vw dev| grep requested_mask=\"wrd|  sed -r 's/.*name="(.*)" pid=.*/\1/' | sort -u |
                while read line
                do
                        p=$(echo "$line")
                        echo "$p"
                        if [ -n "grep $p $DIR/vmm" ]; then
                                #echo "$p"
                                sed -i "\|$(echo $p)|d" $DIR/vmm
                        fi
	...
	$DIR/m.sh
	sed -i 's/audit file,/file,/' ${prof_path}
	cat $DIR/vm | 
	while IFS= read -r line
	do
		#echo "$line"
		#echo -e	"$line\n" >>1.txt
		sed -i "/deny_placeholder,/ a\  ${line}" ${prof_path}
