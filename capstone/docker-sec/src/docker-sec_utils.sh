...
docker-sec-help(){
	echo "Capstone usage:"
	echo "   ${bold_text}capstone COMMAND [ARGUMENTS]${normal_text}"; echo;	#support [OPTIONS] ?
	echo "COMMANDS:"
	...
	echo "    info         Display capstone info and profiles associated with the given container"
...
docker-sec_create(){
	...
	if [ $? -ne 0 -o "${cont_id}" == "-1" ]; then
		echo "${bold_text}Capstone_create:${normal_text} Something went wrong during container creation" >&2
		return 1
	fi
...
docker-sec_run(){
	...
	if [ $? -ne 0 -o "${cont_name}" == "-1" ]; then
		echo "${bold_text}Capstone_run:${normal_text} Something went wrong during container creation" >&2
		return 1
	fi
...
docker-sec_info(){
	shift
	if [ -z $1 ]; then
		echo "usage: ${bold_text}capstone_info${normal_text} CONTAINER_NAME"
		exit $E_NOARGS
	fi
...
docker-sec_train-start_help(){
	echo "usage: ${bold_text}capstone_train-start${normal_text} [OPTIONS] CONTAINER_NAME"
	echo
	echo "OPTIONS:"
...
docker-sec_train-start(){
	...
	if [[ -n $(grep "file[[:space:]]*," /etc/apparmor.d/docker-sec/runtime/${runtime_prof}) ]]; then
		[[ -z $(grep "audit file," /etc/apparmor.d/docker-sec/runtime/${runtime_prof}) ]] &&
                sed -i 's/file,/audit file,/' /etc/apparmor.d/docker-sec/runtime/${runtime_prof}
    else
                sed -i '/shell_placeholder,/ a\  audit file,' /etc/apparmor.d/docker-sec/runtime/${runtime_prof}
    fi
...
docker-sec_train-stop(){
	...
	apply_file_cap_train "${runtime_prof}"
	cp  $DIR/shells_bak $DIR/shells
	echo "" > $DIR/profile
}
...
docker-sec_add(){
	shift
	if [ -z $1 ]; then
		echo "usage: ${bold_text}capstone_add CONTAINER_NAME${normal_text}"
		exit $E_NOARGS
	fi
	...
	container_id=$(container_full_id $1)
	debug_out "add: containerId: ${container_id}"

	runtime_prof="$(run_time_profile_name $container_id)"
	debug_out "add: runtimeProf ${runtime_prof}"
	#runtime_prof_path=/etc/apparmor.d/docker-sec/runtime/${runtime_prof}
	apply_add "${runtime_prof}"
...
docker-sec_rm_help(){
	echo "usage: capstone rm [OPTS] [DOCKER_OPTS] CONTAINER_NAME|CONTAINER_ID${normal_text}";echo
